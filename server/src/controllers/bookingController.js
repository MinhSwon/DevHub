  ...
  try {
    ...
    const slots = await db.query(
      `SELECT t.timeslot_id, t.start_time, t.end_time, t.price
       FROM TimeSlots t
       WHERE t.pitch_id = $1 AND t.day_of_week = $2
    ...
    const bookings = await db.query(
      `SELECT start_time, end_time
       FROM Bookings
       WHERE pitch_id = $1 AND booking_date = $2 AND status IN ('pending','confirmed','completed')`,
      [pitchId, date]
    );

    const isOverlapping = (s1, e1, s2, e2) => s1 < e2 && s2 < e1;
    const availability = slots.rows.map((s) => {
      const taken = bookings.rows.some((b) => isOverlapping(s.start_time, s.end_time, b.start_time, b.end_time));
...
export const createBooking = async (req, res) => {
  ...
  try {
    ...
    const slotRows = await db.query(
      `SELECT price FROM TimeSlots WHERE pitch_id = $1 AND day_of_week = $2 AND start_time = $3 AND end_time = $4`,
      [pitchId, isWeekend, startTime, endTime]
    );
    if (slotRows.rows.length === 0) return res.status(400).json({ error: 'Invalid timeslot' });

    // Check conflict
    const conflicts = await db.query(
      `SELECT booking_id FROM Bookings WHERE pitch_id = $1 AND booking_date = $2 AND status IN ('pending','confirmed','completed')
       AND NOT (end_time <= $3 OR start_time >= $4)`,
      [pitchId, date, startTime, endTime]
    );
    if (conflicts.rows.length > 0) return res.status(409).json({ error: 'Timeslot already booked' });

    const price = slotRows.rows[0].price;
    ...
    res.status(201).json({ bookingId: result.rows[0].booking_id, status: 'pending' });
  } catch (err) {
    console.error(err);
...
export const cancelBooking = async (req, res) => {
  ...
  const { id } = req.params;
  try {
    const rows = await db.query(`SELECT user_id, status FROM Bookings WHERE booking_id = $1`, [id]);
